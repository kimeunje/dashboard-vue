...
# Flask 앱 생성 - Vue 빌드 파일을 정적 파일로 설정
...
def verify_and_login():
    ...
    if email in verification_codes:
        ...
        is_valid_code = is_valid_code or (verification_info["code"] == code and
                                          datetime.now() <= verification_info["expiry"])
...
def serve_vue_app(path):
    ...
    # SPA 라우팅을 위해 모든 경우에 index.html 반환
    try:
        return send_from_directory(app.static_folder, 'index.html')
    except FileNotFoundError:
