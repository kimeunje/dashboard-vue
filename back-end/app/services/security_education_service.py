...
class SecurityEducationService:
                ...
                for i, record in enumerate(records):
                    try:
                        ...
                        period_id = self._get_or_create_education_period(
                            ...
                            record.get('education_type'), uploaded_by)
    ...
    def _find_user_flexible(self, record: dict) -> int:
        ...
        if department:
            user = execute_query(
                ...
                (username, department), fetch_one=True)
            if user:
        ...
        if username_no_space != username:
            user = execute_query(
                ...
                (username_no_space, ), fetch_one=True)
            if user:
    ...
    def get_education_excel_template(self) -> str:
        """엑셀 업로드용 템플릿 생성"""
        template_data = [
            "이름,부서,수강과정,수료,미수료", "테스터,관리실,오프라인,2,0", "홍길동,개발팀,온라인,1,0",
            "김철수,운영팀,오프라인,0,1", "이영희,기획팀,온라인,3,1"
        ]
        return "\n".join(template_data)
            ...
            with DatabaseManager.get_db_cursor() as cursor:
                for idx, record in enumerate(records):
                    try:
                        ...
                        if record_education_type and record_education_type != period_info[
                                'education_type']:
                            print(
                        ...
                        for i in range(completed_count):
                            cursor.execute(
                                ...
                                """,
                                (user_uid, period_id, period_info['education_type'],
    ...
    def _validate_and_get_user(self, username: str, department: str) -> Optional[Dict]:
        ...
        try:
            ...
            user = execute_query(
                ...
                (username.strip(), department.strip()), fetch_one=True)
                ...
                for record in processed_records:
                    try:
                        ...
                        cursor.execute(
                            ...
                            """,
                            (record['user_id'], record['period_id'],
                             record['education_type'], record['education_year'],
                             self._get_education_period_from_date(
                                 record.get('education_date')),
                             record['completion_status'], record.get('education_date'),
                             record['notes'], record['exclude_from_scoring'],
    ...
    def get_education_excel_template(self) -> str:
        ...
        """
        template_data = [
            "# 정보보호 교육 업로드 템플릿", "# 수료: 수료한 교육 횟수, 미수료: 미수료한 교육 횟수",
            "# 교육유형은 선택한 교육 기간의 유형과 일치해야 합니다", "", "이름,부서,수강과정,수료,미수료",
