...
class ManualCheckService:
    ...
    def _save_check_results(self, processed_data, uploaded_by):
        ...
        for data in processed_data:
            try:
                ...
                """, (user_info['uid'], data.get('check_year', 2025),
                      data.get('check_period', 'first_half'), data.get('check_date'),
                      data.get('checker_name', ''), data.get('seal_status'),
                      data.get('seal_number'), data.get('seal_notes'),
                      data.get('malware_scan_result'), data.get('threats_found', 0),
                      data.get('threats_cleaned', False), data.get('antivirus_version'),
                      data.get('malware_notes'), data.get('encryption_status'),
                      data.get('files_scanned', 0), data.get('unencrypted_files', 0),
                      data.get('encryption_completed',
                               False), data.get('encryption_notes'),
                      data.get('overall_result'), total_score, data.get('notes')))
    ...
    def get_check_results(self, year=None, period=None, department=None,
                          result_filter=None, page=1, size=20):
        """임시 간단 버전 - 디버깅용"""
        if year is None:
            year = datetime.now().year

        try:
            # 1. 먼저 manual_check_results만 조회
            simple_query = """
                SELECT 
                    check_id,
                    user_id,
            ...
            print(f"[DEBUG] 간단 쿼리 실행: {year}")
            results = execute_query(simple_query, [year])

            if results:
                print(f"[DEBUG] 첫 번째 결과: {results[0]}")

                # 2. 각 결과에 대해 사용자 정보를 별도로 조회
                enhanced_results = []
                for result in results:
                    user_query = "SELECT user_id, username, department FROM users WHERE uid = %s"
                    user_info = execute_query(user_query, [result['user_id']],
                                              fetch_one=True)

                    enhanced_result = dict(result)
                    if user_info:
                        enhanced_result.update({
                            'user_login_id': user_info['user_id'],
                            'username': user_info['username'],
                            'department': user_info['department']
                        })
                    else:
                        enhanced_result.update({
                            'user_login_id': f"uid_{result['user_id']}",
                            'username': '알 수 없음',
                            'department': '알 수 없음'
                        })
    ...
    def update_check_result(self, check_id, update_data, updated_by):
        ...
        try:
            ...
            execute_query(
                ...
                WHERE check_id = %s
            """,
                (update_data.get('seal_status'), update_data.get('seal_number'),
                 update_data.get('seal_notes'), update_data.get('malware_scan_result'),
                 update_data.get('threats_found',
                                 0), update_data.get('threats_cleaned', False),
                 update_data.get('antivirus_version'), update_data.get('malware_notes'),
                 update_data.get('encryption_status'),
                 update_data.get('files_scanned',
                                 0), update_data.get('unencrypted_files', 0),
                 update_data.get('encryption_completed', False),
                 update_data.get('encryption_notes'), update_data.get('overall_result'),
                 total_score, update_data.get('notes'), check_id))
    ...
    def export_results_to_csv(self, year, period=None):
        ...
        for record in results:
            row = [
                ...
                str(record.get('threats_found',
                               0)), 'Y' if record.get('threats_cleaned') else 'N',
                ...
                str(record.get('unencrypted_files',
                               0)), 'Y' if record.get('encryption_completed') else 'N',
