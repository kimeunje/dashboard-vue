...
class AdminService:
    ...
    def get_admin_dashboard(self) -> dict:
        ...
        if education_stats and education_stats["total_participants"]:
            ...
            completion_rate = (
                education_stats["completed_count"] / total_education_opportunities
            ) * 100
        ...
        if training_stats and training_stats["total_participants"]:
            pass_rate = (
                training_stats["passed_count"] / training_stats["total_participants"]
            ) * 100
        ...
        dashboard_data = {
            ...
            "education": {
                "total_participants": (
                    education_stats["total_participants"] if education_stats else 0
                ),
                "completed": (
                    education_stats["completed_count"] if education_stats else 0
                ),
                "incomplete": (
                    education_stats["incomplete_count"] if education_stats else 0
                ),
            ...
            "training": {
                "total_participants": (
                    training_stats["total_participants"] if training_stats else 0
                ),
        ...
        for user in users:
            try:
                ...
                user["last_score_update"] = (
                    score_info["last_calculated"] if score_info else None
                )
            except:
    ...
    def get_all_education_records(
        self, year: int = None, period: str = None, status: str = None
    ) -> list:
        """모든 교육 기록 조회 (관리자용)"""
        if year is None:
    ...
    def delete_education_record(
        self, user_id: str, education_year: int, education_period: str
    ) -> bool:
        """교육 기록 삭제"""
        try:
            # 사용자 확인
    ...
    def delete_training_record(
        ...
        """모의훈련 기록 삭제"""
        return self.training_service.delete_training_record(
    ...
    def toggle_scoring_exclusion(
        self, user_id: str, year: int, record_type: str, period: str, exclude: bool
    ) -> bool:
        """점수 계산 제외/포함 토글"""
        try:
            # 사용자 ID로 uid 찾기
