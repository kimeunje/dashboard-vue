...
def check_ip_authentication():
    ...
    try:
        ...
        current_app.logger.error(
            f"[CHECK_IP_DEBUG] Raw 데이터 읽기 실패: {str(raw_error)}"
        )
    ...
    try:
        ...
        current_app.logger.error(
            f"[CHECK_IP_DEBUG] 스택 트레이스: {traceback.format_exc()}"
        )
...
def verify_and_login():
    ...
    if not auth_result["success"]:
        ...
        return (
            jsonify({"success": False, "message": auth_result["message"]}),
    ...
    if auth_result["username"] != username:
        ...
        return (
            jsonify({"success": False, "message": "인증 정보가 일치하지 않습니다."}),
            HTTP_STATUS["UNAUTHORIZED"],
        )
...
def authenticate():
    ...
    try:
        ...
        current_app.logger.info(
            f"[AUTHENTICATE_DEBUG] authenticate_user_in_db 호출 시작"
        )
        ...
        if result["success"]:
            ...
            current_app.logger.info(
                f"[AUTHENTICATE_DEBUG] === /authenticate 성공 완료 ==="
            )
            return jsonify(response_data)
        else:
            current_app.logger.warning(
            ...
            current_app.logger.warning(
                f"[AUTHENTICATE_DEBUG] 실패 응답: {error_response}"
            )
            current_app.logger.info(
                f"[AUTHENTICATE_DEBUG] === /authenticate 실패 완료 ==="
            )
            return jsonify(error_response), status_code

    except Exception as e:
        current_app.logger.error(
            f"[AUTHENTICATE_DEBUG] /authenticate 예외 발생: {str(e)}"
        )
        import traceback
        ...
        current_app.logger.error(
            f"[AUTHENTICATE_DEBUG] === /authenticate 예외 완료 ==="
        )
...
@handle_exceptions
def get_ip_info():
    """현재 클라이언트 IP 정보 조회 (디버깅/테스트용) - 강화된 디버깅"""
    current_app.logger.info(f"[IP_INFO_DEBUG] === /ip-info 요청 시작 ===")
    ...
    current_app.logger.info(f"[IP_INFO_DEBUG] 최종 클라이언트 IP: {client_ip}")
    current_app.logger.info(f"[IP_INFO_DEBUG] 모든 IP 헤더: {ip_headers}")
    ...
    current_app.logger.info(f"[IP_INFO_DEBUG] === /ip-info 완료 ===")
    return jsonify(response_data)
