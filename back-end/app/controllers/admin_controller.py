...
def delete_education_record():
    ...
    try:
        success = admin_service.delete_education_record(data['user_id'],
                                                        data['education_year'],
...
def delete_training_record():
    ...
    try:
        success = admin_service.delete_training_record(data['user_id'],
                                                       data['training_year'],
...
def toggle_scoring_exclusion():
    ...
    try:
        success = admin_service.toggle_scoring_exclusion(data['user_id'], data['year'],
                                                         data['record_type'],
                                                         data['period'],
                                                         data['exclude'])
        if success:
            ...
            return jsonify({"error": "설정 변경에 실패했습니다."}), HTTP_STATUS['BAD_REQUEST']
    except ValueError as e:
        return jsonify({"error": str(e)}), HTTP_STATUS['BAD_REQUEST']
...
def export_training_data():
    ...
    try:
        ...
        if format_type == 'csv':
            ...
            csv_data = []
            headers = [
                '사용자ID', '사용자명', '부서', '연도', '기간', '발송시각', '수행시각', '로그유형', '메일유형',
                '이메일', 'IP주소', '결과', '점수', '점수제외', '비고'
            ]
