...
def get_users_list():
    ...
    try:
        ...
        # 필터링된 사용자 데이터 조회
        users_data, total_count = _get_filtered_users(year, department, position,
...
def export_users():
    ...
    try:
        if export_type == "selected" and user_ids:
            ...
            # 필터된 사용자 전체 내보내기
            return _export_filtered_users(year, department, position, risk_level,
                                          search, format_type)
        else:
            # 전체 사용자 내보내기
...
def search_users():
    ...
    try:
        ...
        users = execute_query(search_query,
                              (search_term, search_term, search_term, search_term,
                               exact_term, exact_term, exact_term, limit))
...
def _get_filtered_users(year, department, position, risk_level, search, page, per_page,
                        sort_by, sort_order):
    """필터링된 사용자 목록 조회"""
    try:
        # WHERE 조건 구성
        ...
        if search:
            where_conditions.append(
                "(u.name LIKE %s OR u.user_id LIKE %s OR u.email LIKE %s)")
...
def _export_selected_users(user_ids, year, format_type):
    ...
    try:
        ...
        return _create_csv_response(users_data,
                                    f"selected_users_{len(user_ids)}_members_{year}")
...
def _export_all_users(year, format_type):
    """전체 사용자 내보내기"""
    try:
        users_data, _ = _get_filtered_users(year, "", "", "", "", 1, 999999,
                                            "total_penalty", "desc")
...
def _create_csv_response(users_data, filename):
    ...
    try:
        ...
        # 헤더
        writer.writerow([
            '이름', '사번', '부서', '직급', '이메일', '총 감점', '감사 감점', '교육 감점', '훈련 감점', '감사 실패',
            '교육 미완료', '훈련 실패', '위험도', '마지막 업데이트'
        ])
        ...
        for user in users_data:
            writer.writerow([
                ...
                user.get('department', ''),
                user.get('position', ''),
                user.get('email', ''), f"{user.get('total_penalty', 0):.2f}",
