...
echo 2.1 암호 복잡도 설정 완료 (20~30초 소요)
:: 방법 1: PowerShell을 이용한 보안 정책 설정
powershell -Command "$tempCfg = [System.IO.Path]::GetTempFileName() + '.cfg'; secedit /export /cfg $tempCfg | Out-Null; if (Test-Path $tempCfg) { (Get-Content $tempCfg) -replace 'PasswordComplexity = 0', 'PasswordComplexity = 1' | Set-Content $tempCfg; secedit /configure /db ([System.IO.Path]::GetTempFileName() + '.sdb') /cfg $tempCfg /areas SECURITYPOLICY | Out-Null; Remove-Item $tempCfg -Force -ErrorAction SilentlyContinue; }"

echo PasswordComplexity = 1 >> "%temp%\pwdpolicy.inf"

secedit /configure /db "%temp%\pwdpolicy.sdb" /cfg "%temp%\pwdpolicy.inf" /areas SECURITYPOLICY /quiet
del "%temp%\pwdpolicy.inf" > nul 2>&1
del "%temp%\pwdpolicy.sdb" > nul 2>&1
...
powershell -Command ^
    ...
    "if ($shares) { " ^
        "foreach ($share in $shares) { " ^
            "try { " ^
                "Write-Host \"공유폴더 '$($share.Name)' 제거 시도...\"; " ^
                "$result = $share.Delete(); " ^
                "if ($result.ReturnValue -eq 0) { " ^
                    "Write-Host \"공유폴더 '$($share.Name)' 제거 완료\"; " ^
                "} else { " ^
                    "Write-Host \"공유폴더 '$($share.Name)' 제거 실패 (코드: $($result.ReturnValue))\"; " ^
                "}; " ^
            "} catch { " ^
                "Write-Host \"공유폴더 '$($share.Name)' 제거 중 오류: $($_.Exception.Message)\"; " ^
            "}; " ^
        "}; " ^
    "} else { " ^
        "Write-Host 'IPC$$ 외에 제거할 공유폴더가 없습니다.' ; " ^
    "};"
