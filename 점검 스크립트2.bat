...
:run_all_security_checks
    ...
    powershell -Command ^
        ...
        "try { " ^
            ...
            "try { $avResponse = (Invoke-RestMethod -Method Post -Uri '%SERVER_URL%/api/security-audit/validate_check' -Body $avBody -ContentType 'application/json; charset=utf-8'); Write-Host '알약 백신 상태 검사 완료' -ForegroundColor Green; } catch { Write-Host '알약 백신 상태 검사 실패 - 네트워크 오류' -ForegroundColor Red; }; " ^
